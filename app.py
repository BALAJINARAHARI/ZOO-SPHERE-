...
def add_event():
  if request.method == 'POST':
    ...
    event_date = datetime.strptime(request.form['event_date'],
                                   '%Y-%m-%d').date()
    event_location = request.form['event_location']
    event_capacity = int(request.form['event_capacity'])
    event_price = int(request.form['event_price'])

    try:
      new_event = Events_fl(name=event_name,
                            date=event_date,
      ...
      return redirect(url_for(
          'manage_events'))  # Redirect to manage events after adding an event
...
def edit_event(event_id):
  if request.method == 'POST':
    ...
    event.date = datetime.strptime(request.form['event_date'],
                                   '%Y-%m-%d').date()
    ...
    try:
      db.session.commit()
      return redirect(
          url_for('manage_events'))  # Redirect to manage events after editing
    except Exception as e:
      return f"An error occurred: {str(e)}"
  else:
    event = Events_fl.query.get(event_id)
  ...
  if event:
    ...
    if num_tickets <= event.capacity - event.tickets_sold:
      ...
      return send_file(pdf_file_path,
                       ...
                       download_name=f'ticket_{event.id}.pdf')
    else:
